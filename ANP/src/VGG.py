...
from utils import cal_lp_norm

class VGG16(nn.Module):
    def __init__(self, enable_lat, layerlist, epsilon,alpha, pro_num, batch_size = 128, num_classes=10, if_dropout = False):
    ...
    def choose_layer(self):
        ...
        if self.layerlist == 'all':
            self.enable_list = [1 for i in range(14)]   # all True
        else:
    ...
    def save_grad(self):
        if self.enable_lat:
            for i in self.layerlist_digit:  # [0,1,4,7]
                if i != 0:
                    exec('self.z{}_reg.data = self.alpha * self.z{}_reg.data + self.z{}.grad / cal_lp_norm(self.z{}.grad,p=2,dim_count=len(self.z{}.grad.size()))'.format(i,i,i,i,i))
                else:
                    self.x_reg.data = self.alpha * self.x_reg.data + \
                        self.input.grad / cal_lp_norm(self.input.grad,p=2,dim_count=len(self.input.grad.size()))
