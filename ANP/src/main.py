...
parser.add_argument('--layerlist', default="all", help='layerlist of lat')
...
parser.add_argument('--test_data_path', default="/test/clean/test_data_cln.p", help='test data path')
parser.add_argument('--test_label_path', default="/test/clean/test_label.p", help='test label path')
parser.add_argument('--train_data_path', default="/cifar-10/", help='training dataset path')
parser.add_argument('--model_path', default="/vgg16_all/", help='number of classes')
...
parser.add_argument('--dataset', default="cifar10", help='data set')
parser.add_argument('--model', default="vgg16", help='target model, [lenet, resnet, vgg, ...]')
parser.add_argument('--logfile',default='log/vgg16_all.txt', help='log file to accord validation process')
parser.add_argument('--enable_noise', type=get_bool, default=False, help='enable gaussian noise on gradient')
parser.add_argument('--loss_acc_path',default='loss_acc/vgg16_all/',help='save train loss as .p to draw pic')
...
def train_op(model):
    ...
    if args.dataset == 'cifar10':
        ...
        test_x = torch.Tensor(test_data.data).reshape(-1, 3, 32, 32)[:args.batchsize] / 255.
        test_y = torch.Tensor(test_data.targets)[:args.batchsize]
    ...
    for epoch in range(args.epoch):
        ...
        for step, (x, y) in enumerate(train_loader):
            ...
            if not len(y) == args.batchsize:
                continue
            b_x = Variable(x) # b_x = Variable(x).cuda()
            b_y = Variable(y) # b_y = Variable(y).cuda()
			# enable anp training
            if args.enable_lat:
                model.zero_reg()
            # progressive process
            ...
            if step % 20 == 0:
                ...
                pred_y = torch.max(test_output, 1)[1].data.squeeze().numpy()# pred_y = torch.max(test_output, 1)[1].cuda().data.cpu().squeeze().numpy()
